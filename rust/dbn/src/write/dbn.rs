...
mod tests {
        ...
        T: ConstRType + Clone,
    {
        ...
        write_dbn_stream(writer, VecStream::new(records.clone())).unwrap();
        dbg!(&buffer);
        let metadata = Metadata {
            version: 1,
            dataset: "GLBX.MDP3".to_owned(),
        ...
        T: ConstRType + Clone + fmt::Debug + PartialEq,
    {
        ...
        let mut iter: DbnStreamIter<&[u8], T> =
            DbnStreamIter::new(buffer.as_slice(), metadata).unwrap();
        let mut res = Vec::new();
    ...
    #[test]
    fn test_decode_malformed_encoded_dbn() {
        let records = vec![
            OhlcvMsg {
                hd: RecordHeader {
                    rtype: rtype::OHLCV,
        ...
        type WrongRecord = MboMsg;
        let mut iter: DbnStreamIter<&[u8], WrongRecord> =
            DbnStreamIter::new(buffer.as_slice(), metadata).unwrap();
        // check doesn't panic
