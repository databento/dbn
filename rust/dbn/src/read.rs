...
#[derive(Debug)]
pub struct Dbn<R: io::BufRead> {
    reader: R,
    metadata: Metadata,
}

/// Information about the data contained in a Zstd-compressed DBN file.
...
impl Dbn<BufReader<File>> {
    /// Creates a new [`Dbn`] from the file at `path`. This function reads the metadata,
...
impl<R: io::BufRead> Dbn<R> {
    ...
    /// Returns the [`Schema`] of the DBN data. The schema also indicates the record type `T` for
    /// [`Self::try_into_iter`].
    pub fn schema(&self) -> Schema {
        self.metadata.schema
    }

    /// Returns a reference to all metadata read from the Dbn data in a [`Metadata`] object.
    pub fn metadata(&self) -> &Metadata {
        &self.metadata
    }

    /// Try to decode the DBN file into a streaming iterator. This decodes the
    /// data lazily.
    ///
    /// # Errors
...
/// A consuming iterator over a [`Dbn`]. Lazily decompresses and translates the contents of the file
/// or other buffer. This struct is created by the [`Dbn::try_into_iter`] method.
pub struct DbnStreamIter<R: io::BufRead, T> {
    /// [`Metadata`] about the file being iterated
    metadata: Metadata,
    /// Reference to the underlying [`Dbn`] object.
    /// Buffered zstd decoder of the DBN file, so each call to [`DbnStreamIter::next()`] doesn't result in a
    ...
    buffer: Vec<u8>,
    /// Required to associate [`DbnStreamIter`] with a `T`.
    _item: PhantomData<T>,
}

impl<R: io::BufRead, T> DbnStreamIter<R, T> {
...
impl<R: io::BufRead, T: ConstTypeId> StreamingIterator for DbnStreamIter<R, T> {
    ...
    fn advance(&mut self) {
        if let Err(e) = self.decoder.read_exact(&mut self.buffer) {
            warn!("Failed to read from DBN decoder: {e:?}");
...
impl Metadata {
    ...
    fn decode(metadata_buffer: Vec<u8>) -> anyhow::Result<Self> {
        ...
        let mut pos = 0;
        // TODO(cg): Remove support for DBZ version string in later version
        if !matches!(&metadata_buffer[pos..pos + 3], b"DBN" | b"DBZ") {
        ...
        if version > SCHEMA_VERSION {
            return Err(anyhow!("Can't read newer version of DBN"));
        }
...
mod tests {
    ...
    macro_rules! test_reading_dbn {
        ...
        ($test_name:ident, $record_type:ident, $schema:expr) => {
            ...
            fn $test_name() {
                let target =
                    Dbn::from_file(format!("{DBN_PATH}/test_data.{}.dbn.zst", $schema.as_str()))
                        .unwrap();
    ...
    test_reading_dbn!(test_reading_mbo, MboMsg, Schema::Mbo);
    test_reading_dbn!(test_reading_mbp1, Mbp1Msg, Schema::Mbp1);
    test_reading_dbn!(test_reading_mbp10, Mbp10Msg, Schema::Mbp10);
    test_reading_dbn!(test_reading_ohlcv1d, OhlcvMsg, Schema::Ohlcv1D);
    test_reading_dbn!(test_reading_ohlcv1h, OhlcvMsg, Schema::Ohlcv1H);
    test_reading_dbn!(test_reading_ohlcv1m, OhlcvMsg, Schema::Ohlcv1M);
    test_reading_dbn!(test_reading_ohlcv1s, OhlcvMsg, Schema::Ohlcv1S);
    test_reading_dbn!(test_reading_tbbo, TbboMsg, Schema::Tbbo);
    test_reading_dbn!(test_reading_trades, TradeMsg, Schema::Trades);
    test_reading_dbn!(
