...
pub struct Args {
    #[clap(
        help = "A DBN file to convert to another encoding. Pass '-' to read from standard input",
        value_name = "FILE"
    )]
    pub input: PathBuf,
    #[clap(
    ...
    #[clap(
        ...
        action = ArgAction::SetTrue,
        default_value = "false",
        help = "Output the metadata section instead of the body of the DBN file"
    )]
...
pub fn infer_encoding(args: &Args) -> anyhow::Result<dbn::OutputEncoding> {
    match args.output_encoding() {
        OutputEncoding::Csv => Ok(dbn::OutputEncoding::Csv),
        OutputEncoding::Json => Ok(dbn::OutputEncoding::Json {
            should_pretty_print: args.should_pretty_print,
        }),
        OutputEncoding::Infer => match args.output.as_ref().and_then(|o| o.extension()) {
            Some(ext) if ext == "csv" => Ok(dbn::OutputEncoding::Csv),
            Some(ext) if ext == "json" => Ok(dbn::OutputEncoding::Json {
                should_pretty_print: args.should_pretty_print,
            }),
            Some(ext) => Err(anyhow!(
